...
public class SaleEaoImpl implements SaleEao {
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public Boolean checkExistSale(Integer nuicInsured, Date dateOfSale, Short bankId, Short productId,
			Short collectionPeriodId) throws EaoException {

		try {

			String query = "SELECT s.id FROM " + "Sale s " + "left join s.bank b " + "left join s.product p "
					+ "left join s.collectionPeriod cp " + "WHERE s.nuicInsured = :nuicInsured "
					+ "and s.date = :dateOfSale " + "and p.id  = :productId " + "and b.id = :bankId "
